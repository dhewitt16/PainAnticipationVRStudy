...
% runPSD = 1; %1 if you want to compute power spectra
...
%if runPSD == 1
    ...
    ERDall = zeros(32,numel([freqStart:freqEnd]),numel([epochStart:epochSteps:epochEnd]),numel(conds),numel(cues),0);
   % GERDall = zeros(32,numel([freqStart:freqEnd]),numel([epochStart:epochSteps:epochEnd]),numel(conds),numel(cues),0,numel(sub));
    ...
    for iSub=1:numel(sub)
        ...
        for iCond=1:numel(conds)
            for iCue = 1:numel(cues)
                ...
                RestVals = mean(TFR.powspctrm(:,:,:,bt1:bt2),4); RestVals = squeeze(mean(RestVals,1));

                %computing ERD now with 100*(a-r)/r formula
                erd = zeros(32,numel(freqStart:freqEnd),numel(epochStart:epochSteps:epochEnd), size(TFR.powspctrm,1));
                for iTrial = 1:size(TFR.powspctrm,1)
                    for els=1:32
                        for freqs=1:size(TFR.powspctrm,3)
                            for tms = 1:size(TFR.powspctrm,4)
                                erd(els,freqs,tms,iTrial) = 100*((TFR.powspctrm(iTrial,els,freqs,tms)-RestVals(els,freqs))./RestVals(els,freqs)); %this is the Pfu's formula 100*(A-R)/R
                            end
                        end
                    end
                end

                ERDall(:,:,:,iCond,iCue, end+1:end+size(erd, 4)) = erd;
                % GERDall(:,:,:,iCond,iCue,iSub) = erd; %append the data to a matrix of channels x freqs x timepoints x conds x participants

            end
        end

        save(subOutname,'ERDall', '-v7.3'); %saving per subject - ERD
        PPSfreq= TFR.freq;
    ...
    %
    %     allSubOutname = [fullfile(dir, ['AllERD_withcsd_bl21_dpss_nofqbl_st_' d '.mat'])];
...
%     end
%
%     outname = [dir 'Exports/ERD/PPSERDDataWide_noav_st_180324.csv'];
...
%
%     outname = [dir 'Exports/ERD/PPSERDDataLong_noav_st_180324.csv'];
...
%     end
%
%     outname = [dir 'Exports/ERD/PPSERDDataWide_st_180324.csv'];
%     writematrix(bigexport,outname);
...
%     stackedTable(:, 'BlockCue') = [];
%
%     outname = [dir 'Exports/ERD/PPSERDDataLong_st_180324.csv'];
%     writetable(stackedTable,outname);
%     disp(['Averaged results saved to ' outname])
%
%
