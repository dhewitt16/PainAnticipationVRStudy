...
% Danielle Hewitt (10/08/2023) MOST RECENT VERSION - USE!!!
...
for iSub = 1:size(cfg.sub,2)
    ...
    currentDirectory = [cfg.dir 'P' currentSubject '/'];
    ...
    for iCond = 1:4 %loop to run all conditions
       ...
       %% Recoding Event Triggers
        ...
        EEG = eeg_checkset( EEG );
        %   pop_saveset(EEG,'filename',['P' currentSubject '_' currentCond
        %   '_preproc.set'],'filepath',currentDirectory);         % uncomment to save the individual cond data file
    ...
    MEEG = pop_eegfiltnew(MEEG, 'locutoff',1,'hicutoff',30); MEEG = eeg_checkset( MEEG ); %filter 1-70 hz
    %  MEEG = pop_resample(MEEG,100); %this currently isn't working -
    %  find out why?
    ...
    % pop_saveset(MEEG,'filename',['P' currentSubject_
    % '_forICA.set'],'filepath',currentDirectory); %saving the setfile
...
%     % %==========================================================================
    ...
    for iCond = 1:4 %loop to run all conditions
        ...
        EEG = eeg_checkset( EEG );
        %long epoch
        % EEG = pop_epoch( EEG, {  'left1' 'left2' 'left3' 'left4' 'right1' 'right2' 'right3' 'right4' 'middle1' 'middle2' 'middle3' 'middle4'}, [-2.5  5.5], 'newname', 'Epoched data', 'epochinfo', 'yes');
       
        %short epoch
        EEG = pop_epoch( EEG, {  'left1' 'left2' 'left3' 'left4' 'right1' 'right2' 'right3' 'right4' 'middle1' 'middle2' 'middle3' 'middle4'}, [-2.5  2.5], 'newname', 'Epoched data', 'epochinfo', 'yes');
      
        [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, CURRENTSET, 'setname', 'Continuous EEG Data epochs');
        eeglab redraw;

        EEG = eeg_checkset( EEG );
        pop_saveset(EEG,'filename',['P' currentSubject '_allepoched.set'],'filepath',currentDirectory); %saving the file ready for ICA rejection and artifact rejection
